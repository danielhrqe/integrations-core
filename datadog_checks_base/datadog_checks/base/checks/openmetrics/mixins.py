...
class OpenMetricsScraperMixin(object):
    ...
    def create_scraper_configuration(self, instance=None):
        ...
        # `_cached_type_overrides` is a dictionary where we cache the known metric names
        # that match (or don't match) the `type_overrides` wildcard configurations.
        # Using `_cached_type_overrides` will avoid calling fnmatch in every check run.
        config['_cached_type_overrides'] = {}
    ...
    def parse_metric_family(self, response, scraper_config):
        ...
        for metric in text_fd_to_metric_families(input_gen):
            ...
            cached_type_override = scraper_config['_cached_type_overrides'].get(metric.name)
            if type_override:
                metric.type = type_override
            elif cached_type_override:
                metric.type = cached_type_override
                ...
                for name_pattern, new_type in iteritems(scraper_config['type_overrides']):
                    if fnmatch(metric.name, name_pattern):
                        ...
                        break
                scraper_config['_cached_type_overrides'][metric.name] = metric.type
